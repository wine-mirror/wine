...
static inline struct d3d11_device_context *impl_from_ID3D11DeviceContext3(ID3D11DeviceContext3 *iface)
{
    return CONTAINING_RECORD(iface, struct d3d11_device_context, ID3D11DeviceContext3_iface);
}

static HRESULT STDMETHODCALLTYPE d3d11_device_context_QueryInterface(ID3D11DeviceContext3 *iface,
        REFIID iid, void **out)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);

    TRACE("iface %p, iid %s, out %p.\n", iface, debugstr_guid(iid), out);

    if (IsEqualGUID(iid, &IID_ID3D11DeviceContext3)
            ...
            || IsEqualGUID(iid, &IID_IUnknown))
    {
        *out = &context->ID3D11DeviceContext3_iface;
    }
    ...
    ID3D11DeviceContext3_AddRef(iface);
    return S_OK;
}

static ULONG STDMETHODCALLTYPE d3d11_device_context_AddRef(ID3D11DeviceContext3 *iface)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static ULONG STDMETHODCALLTYPE d3d11_device_context_Release(ID3D11DeviceContext3 *iface)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static void d3d11_device_context_get_constant_buffers(ID3D11DeviceContext3 *iface, enum wined3d_shader_type type,
...
static void d3d11_device_context_set_constant_buffers(ID3D11DeviceContext3 *iface, enum wined3d_shader_type type,
...
static void d3d11_device_context_set_shader_resource_views(ID3D11DeviceContext3 *iface, enum wined3d_shader_type type,
...
static void d3d11_device_context_set_samplers(ID3D11DeviceContext3 *iface, enum wined3d_shader_type type,
...
static void STDMETHODCALLTYPE d3d11_device_context_GetDevice(ID3D11DeviceContext3 *iface, ID3D11Device **device)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static HRESULT STDMETHODCALLTYPE d3d11_device_context_GetPrivateData(ID3D11DeviceContext3 *iface, REFGUID guid,
        UINT *data_size, void *data)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static HRESULT STDMETHODCALLTYPE d3d11_device_context_SetPrivateData(ID3D11DeviceContext3 *iface, REFGUID guid,
        UINT data_size, const void *data)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static HRESULT STDMETHODCALLTYPE d3d11_device_context_SetPrivateDataInterface(ID3D11DeviceContext3 *iface,
        REFGUID guid, const IUnknown *data)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static void STDMETHODCALLTYPE d3d11_device_context_VSSetConstantBuffers(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_PSSetShaderResources(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_PSSetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_PSSetSamplers(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_VSSetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_DrawIndexed(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_Draw(ID3D11DeviceContext3 *iface,
...
static HRESULT STDMETHODCALLTYPE d3d11_device_context_Map(ID3D11DeviceContext3 *iface, ID3D11Resource *resource,
...
static void STDMETHODCALLTYPE d3d11_device_context_Unmap(ID3D11DeviceContext3 *iface, ID3D11Resource *resource,
...
static void STDMETHODCALLTYPE d3d11_device_context_PSSetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer *const *buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_IASetInputLayout(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_IASetVertexBuffers(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_IASetIndexBuffer(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_DrawIndexedInstanced(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_DrawInstanced(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_GSSetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer *const *buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_GSSetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_IASetPrimitiveTopology(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_VSSetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView *const *views)
{
    TRACE("iface %p, start_slot %u, view_count %u, views %p.\n", iface, start_slot, view_count, views);
...
static void STDMETHODCALLTYPE d3d11_device_context_VSSetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState *const *samplers)
{
    TRACE("iface %p, start_slot %u, sampler_count %u, samplers %p.\n",
            iface, start_slot, sampler_count, samplers);
...
static void STDMETHODCALLTYPE d3d11_device_context_Begin(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_End(ID3D11DeviceContext3 *iface,
        ID3D11Asynchronous *asynchronous)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    struct d3d_query *query = unsafe_impl_from_ID3D11Asynchronous(asynchronous);
...
static HRESULT STDMETHODCALLTYPE d3d11_device_context_GetData(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_SetPredication(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_GSSetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView *const *views)
{
    TRACE("iface %p, start_slot %u, view_count %u, views %p.\n", iface, start_slot, view_count, views);
...
static void STDMETHODCALLTYPE d3d11_device_context_GSSetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState *const *samplers)
{
    TRACE("iface %p, start_slot %u, sampler_count %u, samplers %p.\n",
            iface, start_slot, sampler_count, samplers);
...
static void STDMETHODCALLTYPE d3d11_device_context_OMSetRenderTargets(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_OMSetRenderTargetsAndUnorderedAccessViews(
        ID3D11DeviceContext3 *iface, UINT render_target_view_count, ID3D11RenderTargetView *const *render_target_views,
...
static void STDMETHODCALLTYPE d3d11_device_context_OMSetBlendState(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_OMSetDepthStencilState(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_SOSetTargets(ID3D11DeviceContext3 *iface, UINT buffer_count,
...
static void STDMETHODCALLTYPE d3d11_device_context_DrawAuto(ID3D11DeviceContext3 *iface)
{
    FIXME("iface %p stub!\n", iface);
}

static void STDMETHODCALLTYPE d3d11_device_context_DrawIndexedInstancedIndirect(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_DrawInstancedIndirect(ID3D11DeviceContext3 *iface,
        ID3D11Buffer *buffer, UINT offset)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    struct d3d_buffer *d3d_buffer;
...
static void STDMETHODCALLTYPE d3d11_device_context_Dispatch(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_DispatchIndirect(ID3D11DeviceContext3 *iface,
        ID3D11Buffer *buffer, UINT offset)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    struct d3d_buffer *buffer_impl;
...
static void STDMETHODCALLTYPE d3d11_device_context_RSSetState(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_RSSetViewports(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_RSSetScissorRects(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CopySubresourceRegion(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CopyResource(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_UpdateSubresource(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CopyStructureCount(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_ClearRenderTargetView(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_ClearUnorderedAccessViewUint(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_ClearUnorderedAccessViewFloat(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_ClearDepthStencilView(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_GenerateMips(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_SetResourceMinLOD(ID3D11DeviceContext3 *iface,
        ID3D11Resource *resource, FLOAT min_lod)
{
    FIXME("iface %p, resource %p, min_lod %f stub!\n", iface, resource, min_lod);
}

static FLOAT STDMETHODCALLTYPE d3d11_device_context_GetResourceMinLOD(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_ResolveSubresource(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_ExecuteCommandList(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_HSSetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView *const *views)
{
    TRACE("iface %p, start_slot %u, view_count %u, views %p.\n",
            iface, start_slot, view_count, views);
...
static void STDMETHODCALLTYPE d3d11_device_context_HSSetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_HSSetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState *const *samplers)
{
    TRACE("iface %p, start_slot %u, sampler_count %u, samplers %p.\n",
            iface, start_slot, sampler_count, samplers);
...
static void STDMETHODCALLTYPE d3d11_device_context_HSSetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer *const *buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_DSSetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView *const *views)
{
    TRACE("iface %p, start_slot %u, view_count %u, views %p.\n",
            iface, start_slot, view_count, views);
...
static void STDMETHODCALLTYPE d3d11_device_context_DSSetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_DSSetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState *const *samplers)
{
    TRACE("iface %p, start_slot %u, sampler_count %u, samplers %p.\n",
            iface, start_slot, sampler_count, samplers);
...
static void STDMETHODCALLTYPE d3d11_device_context_DSSetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer *const *buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_CSSetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView *const *views)
{
    TRACE("iface %p, start_slot %u, view_count %u, views %p.\n",
            iface, start_slot, view_count, views);
...
static void STDMETHODCALLTYPE d3d11_device_context_CSSetUnorderedAccessViews(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CSSetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CSSetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState *const *samplers)
{
    TRACE("iface %p, start_slot %u, sampler_count %u, samplers %p.\n",
            iface, start_slot, sampler_count, samplers);
...
static void STDMETHODCALLTYPE d3d11_device_context_CSSetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer *const *buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_VSGetConstantBuffers(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_PSGetShaderResources(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_PSGetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_PSGetSamplers(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_VSGetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_PSGetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_IAGetInputLayout(ID3D11DeviceContext3 *iface,
        ID3D11InputLayout **input_layout)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static void STDMETHODCALLTYPE d3d11_device_context_IAGetVertexBuffers(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_IAGetIndexBuffer(ID3D11DeviceContext3 *iface,
        ID3D11Buffer **buffer, DXGI_FORMAT *format, UINT *offset)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    enum wined3d_format_id wined3d_format;
...
static void STDMETHODCALLTYPE d3d11_device_context_GSGetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_GSGetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_IAGetPrimitiveTopology(ID3D11DeviceContext3 *iface,
        D3D11_PRIMITIVE_TOPOLOGY *topology)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    enum wined3d_primitive_type primitive_type;
...
static void STDMETHODCALLTYPE d3d11_device_context_VSGetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView **views)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_VSGetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState **samplers)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_GetPredication(ID3D11DeviceContext3 *iface,
        ID3D11Predicate **predicate, BOOL *value)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static void STDMETHODCALLTYPE d3d11_device_context_GSGetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView **views)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_GSGetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState **samplers)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_OMGetRenderTargets(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_OMGetBlendState(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_OMGetDepthStencilState(ID3D11DeviceContext3 *iface,
        ID3D11DepthStencilState **depth_stencil_state, UINT *stencil_ref)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static void STDMETHODCALLTYPE d3d11_device_context_SOGetTargets(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_RSGetState(ID3D11DeviceContext3 *iface,
        ID3D11RasterizerState **rasterizer_state)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    struct d3d_rasterizer_state *rasterizer_state_impl;
...
static void STDMETHODCALLTYPE d3d11_device_context_RSGetViewports(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_RSGetScissorRects(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_HSGetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView **views)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_HSGetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_HSGetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState **samplers)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_HSGetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_DSGetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView **views)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_DSGetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_DSGetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState **samplers)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_DSGetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_CSGetShaderResources(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT view_count, ID3D11ShaderResourceView **views)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_CSGetUnorderedAccessViews(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CSGetShader(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CSGetSamplers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT sampler_count, ID3D11SamplerState **samplers)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
    unsigned int i;
...
static void STDMETHODCALLTYPE d3d11_device_context_CSGetConstantBuffers(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p.\n",
            iface, start_slot, buffer_count, buffers);
...
static void STDMETHODCALLTYPE d3d11_device_context_ClearState(ID3D11DeviceContext3 *iface)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static void STDMETHODCALLTYPE d3d11_device_context_Flush(ID3D11DeviceContext3 *iface)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static D3D11_DEVICE_CONTEXT_TYPE STDMETHODCALLTYPE d3d11_device_context_GetType(ID3D11DeviceContext3 *iface)
{
    struct d3d11_device_context *context = impl_from_ID3D11DeviceContext3(iface);
...
static UINT STDMETHODCALLTYPE d3d11_device_context_GetContextFlags(ID3D11DeviceContext3 *iface)
{
    TRACE("iface %p.\n", iface);
...
static HRESULT STDMETHODCALLTYPE d3d11_device_context_FinishCommandList(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CopySubresourceRegion1(ID3D11DeviceContext3 *iface,
        ID3D11Resource *dst_resource, UINT dst_subresource_idx, UINT dst_x, UINT dst_y, UINT dst_z,
...
static void STDMETHODCALLTYPE d3d11_device_context_UpdateSubresource1(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_DiscardResource(ID3D11DeviceContext3 *iface,
        ID3D11Resource *resource)
{
    FIXME("iface %p, resource %p stub!\n", iface, resource);
}

static void STDMETHODCALLTYPE d3d11_device_context_DiscardView(ID3D11DeviceContext3 *iface, ID3D11View *view)
{
    FIXME("iface %p, view %p stub!\n", iface, view);
}

static void STDMETHODCALLTYPE d3d11_device_context_VSSetConstantBuffers1(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_HSSetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer * const *buffers, const UINT *first_constant,
        const UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
...
static void STDMETHODCALLTYPE d3d11_device_context_DSSetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer * const *buffers, const UINT *first_constant,
        const UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
...
static void STDMETHODCALLTYPE d3d11_device_context_GSSetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer * const *buffers, const UINT *first_constant,
        const UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
...
static void STDMETHODCALLTYPE d3d11_device_context_PSSetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer * const *buffers, const UINT *first_constant,
        const UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
...
static void STDMETHODCALLTYPE d3d11_device_context_CSSetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer * const *buffers, const UINT *first_constant,
        const UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
...
static void STDMETHODCALLTYPE d3d11_device_context_VSGetConstantBuffers1(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_HSGetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers, UINT *first_constant, UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
            iface, start_slot, buffer_count, buffers, first_constant, num_constants);
...
static void STDMETHODCALLTYPE d3d11_device_context_DSGetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers, UINT *first_constant, UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
            iface, start_slot, buffer_count, buffers, first_constant, num_constants);
...
static void STDMETHODCALLTYPE d3d11_device_context_GSGetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers, UINT *first_constant, UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
            iface, start_slot, buffer_count, buffers, first_constant, num_constants);
...
static void STDMETHODCALLTYPE d3d11_device_context_PSGetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers, UINT *first_constant, UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
            iface, start_slot, buffer_count, buffers, first_constant, num_constants);
...
static void STDMETHODCALLTYPE d3d11_device_context_CSGetConstantBuffers1(ID3D11DeviceContext3 *iface,
        UINT start_slot, UINT buffer_count, ID3D11Buffer **buffers, UINT *first_constant, UINT *num_constants)
{
    TRACE("iface %p, start_slot %u, buffer_count %u, buffers %p, first_constant %p, num_constants %p.\n",
            iface, start_slot, buffer_count, buffers, first_constant, num_constants);
...
static void STDMETHODCALLTYPE d3d11_device_context_SwapDeviceContextState(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_ClearView(ID3D11DeviceContext3 *iface, ID3D11View *view,
...
static void STDMETHODCALLTYPE d3d11_device_context_DiscardView1(ID3D11DeviceContext3 *iface, ID3D11View *view,
        const D3D11_RECT *rects, UINT num_rects)
{
    FIXME("iface %p, view %p, rects %p, num_rects %u stub!\n", iface, view, rects, num_rects);
}

static HRESULT STDMETHODCALLTYPE d3d11_device_context_UpdateTileMappings(ID3D11DeviceContext3 *iface,
...
static HRESULT STDMETHODCALLTYPE d3d11_device_context_CopyTileMappings(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_CopyTiles(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_UpdateTiles(ID3D11DeviceContext3 *iface,
        ID3D11Resource *dst_resource, const D3D11_TILED_RESOURCE_COORDINATE *dst_start_coordinate,
...
static HRESULT STDMETHODCALLTYPE d3d11_device_context_ResizeTilePool(ID3D11DeviceContext3 *iface,
...
static void STDMETHODCALLTYPE d3d11_device_context_TiledResourceBarrier(ID3D11DeviceContext3 *iface,
...
static BOOL STDMETHODCALLTYPE d3d11_device_context_IsAnnotationEnabled(ID3D11DeviceContext3 *iface)
{
    FIXME("iface %p stub!\n", iface);
...
static void STDMETHODCALLTYPE d3d11_device_context_SetMarkerInt(ID3D11DeviceContext3 *iface,
        const WCHAR *label, int data)
{
    FIXME("iface %p label %s data %d stub!\n", iface, wine_dbgstr_w(label), data);
}

static void STDMETHODCALLTYPE d3d11_device_context_BeginEventInt(ID3D11DeviceContext3 *iface,
        const WCHAR *label, int data)
{
    FIXME("iface %p label %s data %d stub!\n", iface, wine_dbgstr_w(label), data);
}

static void STDMETHODCALLTYPE d3d11_device_context_EndEvent(ID3D11DeviceContext3 *iface)
{
    FIXME("iface %p stub!\n", iface);
}

static void STDMETHODCALLTYPE d3d11_device_context_Flush1(ID3D11DeviceContext3 *iface,
        D3D11_CONTEXT_TYPE type, HANDLE event)
{
    FIXME("iface %p, type %d, event %p stub!\n", iface, type, event);
}

static void STDMETHODCALLTYPE d3d11_device_context_SetHardwareProtectionState(ID3D11DeviceContext3 *iface,
        BOOL enable)
{
    FIXME("iface %p, enable %d stub!\n", iface, enable);
}

static void STDMETHODCALLTYPE d3d11_device_context_GetHardwareProtectionState(ID3D11DeviceContext3 *iface,
        BOOL *enable)
{
...
static const struct ID3D11DeviceContext3Vtbl d3d11_device_context_vtbl =
{
    /* IUnknown methods */
        ...
        REFIID iid, void **out)
{
    ...
    return d3d11_device_context_QueryInterface(&context->ID3D11DeviceContext3_iface, iid, out);
}
...
static ULONG STDMETHODCALLTYPE d3d11_video_context_AddRef(ID3D11VideoContext *iface)
{
    ...
    return d3d11_device_context_AddRef(&context->ID3D11DeviceContext3_iface);
}
...
static ULONG STDMETHODCALLTYPE d3d11_video_context_Release(ID3D11VideoContext *iface)
{
    ...
    return d3d11_device_context_Release(&context->ID3D11DeviceContext3_iface);
}
...
static void STDMETHODCALLTYPE d3d11_video_context_GetDevice(ID3D11VideoContext *iface, ID3D11Device **device)
{
    ...
    return d3d11_device_context_GetDevice(&context->ID3D11DeviceContext3_iface, device);
}
        ...
        ID3D11VideoContext *iface, REFGUID guid, UINT *size, void *data)
{
    ...
    return d3d11_device_context_GetPrivateData(&context->ID3D11DeviceContext3_iface, guid, size, data);
}
        ...
        ID3D11VideoContext *iface, REFGUID guid, UINT size, const void *data)
{
    ...
    return d3d11_device_context_SetPrivateData(&context->ID3D11DeviceContext3_iface, guid, size, data);
}
        ...
        ID3D11VideoContext *iface, REFGUID guid, const IUnknown *data)
{
    ...
    return d3d11_device_context_SetPrivateDataInterface(&context->ID3D11DeviceContext3_iface, guid, data);
}
        ...
        ID3D11DeviceContext **context)
{
    ...
    *context = (ID3D11DeviceContext *)&object->ID3D11DeviceContext3_iface;
    return S_OK;
}
        ...
        ID3D11DeviceContext **immediate_context)
{
    ...
    *immediate_context = (ID3D11DeviceContext *)&device->immediate_context.ID3D11DeviceContext3_iface;
        ...
        ID3D11DeviceContext1 **immediate_context)
{
    ...
    *immediate_context = (ID3D11DeviceContext1 *)&device->immediate_context.ID3D11DeviceContext3_iface;
        ...
        ID3D11DeviceContext1 **context)
{
    ...
    *context = (ID3D11DeviceContext1 *)&object->ID3D11DeviceContext3_iface;
    return S_OK;
}
        ...
        ID3D10BlendState *blend_state, const float blend_factor[4], UINT sample_mask)
{
    ...
    d3d11_device_context_OMSetBlendState(&device->immediate_context.ID3D11DeviceContext3_iface,
        ...
        ID3D10DepthStencilState *depth_stencil_state, UINT stencil_ref)
{
    ...
    d3d11_device_context_OMSetDepthStencilState(&device->immediate_context.ID3D11DeviceContext3_iface,
...
static void STDMETHODCALLTYPE d3d10_device_RSSetState(ID3D10Device1 *iface, ID3D10RasterizerState *rasterizer_state)
{
    ...
    d3d11_device_context_RSSetState(&device->immediate_context.ID3D11DeviceContext3_iface,
        ...
        const void *data, UINT row_pitch, UINT depth_pitch)
{
    ...
    d3d11_device_context_UpdateSubresource(&device->immediate_context.ID3D11DeviceContext3_iface,
        ...
        ID3D10BlendState **blend_state, FLOAT blend_factor[4], UINT *sample_mask)
{
    ...
    d3d11_device_context_OMGetBlendState(&device->immediate_context.ID3D11DeviceContext3_iface,
        ...
        ID3D10DepthStencilState **depth_stencil_state, UINT *stencil_ref)
{
    ...
    d3d11_device_context_OMGetDepthStencilState(&device->immediate_context.ID3D11DeviceContext3_iface,
...
static void STDMETHODCALLTYPE d3d10_device_ClearState(ID3D10Device1 *iface)
{
    ...
    d3d11_device_context_ClearState(&device->immediate_context.ID3D11DeviceContext3_iface);
}
...
void d3d_device_init(struct d3d_device *device, void *outer_unknown)
{
    ...
    ID3D11DeviceContext3_Release(&device->immediate_context.ID3D11DeviceContext3_iface);
