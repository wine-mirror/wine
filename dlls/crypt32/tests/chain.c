...
static void testGetCertChain(void)
{
    ...
    /* Test a real chain */
    info.Issuer.cbData = sizeof(iTunesIssuer);
    info.Issuer.pbData = iTunesIssuer;
    info.SerialNumber.cbData = sizeof(iTunesSerialNum);
    info.SerialNumber.pbData = iTunesSerialNum;

    store = CertOpenStore(CERT_STORE_PROV_MEMORY, 0, 0,
     CERT_STORE_CREATE_NEW_FLAG, NULL);
    CertAddEncodedCertificateToStore(store, X509_ASN_ENCODING, iTunesCert0,
     sizeof(iTunesCert0), CERT_STORE_ADD_NEW, NULL);
    CertAddEncodedCertificateToStore(store, X509_ASN_ENCODING, iTunesCert1,
     sizeof(iTunesCert1), CERT_STORE_ADD_NEW, NULL);
    CertAddEncodedCertificateToStore(store, X509_ASN_ENCODING, iTunesCert2,
     sizeof(iTunesCert2), CERT_STORE_ADD_NEW, NULL);
    CertAddEncodedCertificateToStore(store, X509_ASN_ENCODING, iTunesCert3,
     sizeof(iTunesCert3), CERT_STORE_ADD_NEW, NULL);

    config.hRestrictedRoot = CertOpenStore(CERT_STORE_PROV_MEMORY, 0, 0,
     CERT_STORE_CREATE_NEW_FLAG, NULL);
    CertAddEncodedCertificateToStore(config.hRestrictedRoot, X509_ASN_ENCODING,
     verisignCA, sizeof(verisignCA), CERT_STORE_ADD_NEW, NULL);
    engine = NULL;
    ret = CertCreateCertificateChainEngine(&config, &engine);

    cert = CertFindCertificateInStore(store, X509_ASN_ENCODING, 0,
     CERT_FIND_SUBJECT_CERT, &info, NULL);
    ok(cert != NULL, "CertFindCertificateInStore failed: %08x\n",
     GetLastError());
    if (cert)
    {
        /* There's no guarantee the Verisign cert is in the root store, in
         * which case the engine creation will fail => we need to pass the
         * memory store that includes the Verisign cert.
         */
        if (engine)
            ret = CertGetCertificateChain(engine, cert, NULL, NULL, &para, 0,
             NULL, &chain);
        else
        ...
        if (ret)
        {
            ...
            ok(chain->TrustStatus.dwInfoStatus ==
             ...
             chain->TrustStatus.dwInfoStatus);
            ok(chain->cChain == 1, "Expected 1 chain, got %d\n", chain->cChain);
            ok(chain->rgpChain[0]->cElement == 3,
             "Expected three chain elements, got %d\n",
             chain->rgpChain[0]->cElement);
            ok(chain->rgpChain[0]->pTrustListInfo == NULL,
             "Expected no trust list\n");
            CertFreeCertificateChain(chain);
    ...
    CertCloseStore(config.hRestrictedRoot, 0);
    CertFreeCertificateChainEngine(engine);
    CertCloseStore(store, 0);
}
