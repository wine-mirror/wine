    ...
    *obj = NULL;
             ...
             IsEqualGUID(riid, &IID_IStream))
    {
        /* FIXME: unidentified service id */
             ...
             (IsEqualGUID(service, &IID_IInternetSecurityManager) && IsEqualGUID(riid, &IID_IInternetSecurityManager)))
    {
    }
    else
...
static IXMLHTTPRequest *create_xhr(void)
{
    ...
    hr = CoCreateInstance(&CLSID_XMLHTTP60, NULL, CLSCTX_INPROC_SERVER, &IID_IXMLHTTPRequest, (void **)&ret);
...
static IServerXMLHTTPRequest *create_server_xhr(void)
{
    ...
    hr = CoCreateInstance(&CLSID_ServerXMLHTTP60, NULL, CLSCTX_INPROC_SERVER, &IID_IServerXMLHTTPRequest, (void **)&ret);
...
static void test_XMLHTTP60(void)
{
    ...
    check_interface(xhr, &IID_IXMLHTTPRequest3, FALSE);
    check_interface(xhr, &IID_IXMLHTTPRequest2, FALSE);
    ...
    todo_wine
    ok(hr == 0xc00c023f, "Unexpected hr %#lx.\n", hr);
    ...
    todo_wine
    ok(hr == 0xc00c023e, "Unexpected hr %#lx.\n", hr);
    ...
    todo_wine
    ok(hr == 0xc00c023f, "Unexpected hr %#lx.\n", hr);
    ok(status == READYSTATE_UNINITIALIZED, "Unexpected state %ld.\n", status);
    ...
    ok(g_expectedcall == 1, "no expected disp event call\n");
    ...
    hr = IXMLHTTPRequest_send(xhr, varbody);
    todo_wine
    ok(hr == 0xc00c0240, "Unexpected hr %#lx.\n", hr);
...
static void test_server_xhr(void)
{
    ...
    /* GET request */
    hr = CoCreateInstance(&CLSID_ServerXMLHTTP60, NULL, CLSCTX_INPROC_SERVER, &IID_IServerXMLHTTPRequest, (void **)&xhr);
    ok(hr == S_OK, "Unexpected hr %#lx.\n", hr);
...
static inline struct xhr_callback *xhr_callback_from_IXMLHTTPRequest3Callback(IXMLHTTPRequest3Callback *iface)
{
    return CONTAINING_RECORD(iface, struct xhr_callback, IXMLHTTPRequest3Callback_iface);
}

static HRESULT WINAPI xhr_callback_QueryInterface(IXMLHTTPRequest3Callback *iface, REFIID riid, void **obj)
{
    if (IsEqualGUID(riid, &IID_IXMLHTTPRequest3Callback)
            || IsEqualGUID(riid, &IID_IXMLHTTPRequest2Callback)
            || IsEqualGUID(riid, &IID_IUnknown))
    {
...
static ULONG WINAPI xhr_callback_AddRef(IXMLHTTPRequest3Callback *iface)
{
    struct xhr_callback *callback = xhr_callback_from_IXMLHTTPRequest3Callback(iface);
    return InterlockedIncrement(&callback->refcount);
}

static ULONG WINAPI xhr_callback_Release(IXMLHTTPRequest3Callback *iface)
{
    struct xhr_callback *callback = xhr_callback_from_IXMLHTTPRequest3Callback(iface);
...
static HRESULT WINAPI xhr_callback_OnRedirect(IXMLHTTPRequest3Callback *iface,
        IXMLHTTPRequest2 *request, const WCHAR *redirect_url)
{
    return S_OK;
}

static HRESULT WINAPI xhr_callback_OnHeadersAvailable(IXMLHTTPRequest3Callback *iface,
        IXMLHTTPRequest2 *request, DWORD status, const WCHAR *status_str)
{
    return S_OK;
}

static HRESULT WINAPI xhr_callback_OnDataAvailable(IXMLHTTPRequest3Callback *iface,
        IXMLHTTPRequest2 *request, ISequentialStream *response)
{
    return S_OK;
}

static HRESULT WINAPI xhr_callback_OnResponseReceived(IXMLHTTPRequest3Callback *iface,
        IXMLHTTPRequest2 *request, ISequentialStream *response)
{
    struct xhr_callback *callback = xhr_callback_from_IXMLHTTPRequest3Callback(iface);
...
static HRESULT WINAPI xhr_callback_OnError(IXMLHTTPRequest3Callback *iface, IXMLHTTPRequest2 *request, HRESULT error)
{
    struct xhr_callback *callback = xhr_callback_from_IXMLHTTPRequest3Callback(iface);
    SetEvent(callback->event);
    return S_OK;
...
static HRESULT WINAPI xhr_callback_OnServerCertificateReceived(IXMLHTTPRequest3Callback *iface,
        IXMLHTTPRequest3 *request, DWORD errors, DWORD chain_size, const XHR_CERT *chain)
{
    return S_OK;
}

static HRESULT WINAPI xhr_callback_OnClientCertificateRequested(IXMLHTTPRequest3Callback *iface,
        IXMLHTTPRequest3 *request, DWORD issuers_size, const WCHAR **issuers)
{
    return S_OK;
}

static const IXMLHTTPRequest3CallbackVtbl xhr_callback_vtbl =
{
    xhr_callback_QueryInterface,
    xhr_callback_AddRef,
    xhr_callback_Release,
    xhr_callback_OnRedirect,
    xhr_callback_OnHeadersAvailable,
    xhr_callback_OnDataAvailable,
    xhr_callback_OnResponseReceived,
    xhr_callback_OnError,
    xhr_callback_OnServerCertificateReceived,
    xhr_callback_OnClientCertificateRequested,
};

static IXMLHTTPRequest2Callback * xhr_create_callback(HANDLE event)
{
    struct xhr_callback *callback = calloc(1, sizeof(*callback));

    callback->IXMLHTTPRequest3Callback_iface.lpVtbl = &xhr_callback_vtbl;
    callback->refcount = 1;
    callback->event = event;

    return (IXMLHTTPRequest2Callback *)&callback->IXMLHTTPRequest3Callback_iface;
}

static void test_FreeThreadedXMLHTTP60(void)
{
    ...
    hr = CoCreateInstance(&CLSID_FreeThreadedXMLHTTP60, NULL, CLSCTX_INPROC_SERVER, &IID_IXMLHTTPRequest2, (void **)&xhr);
    if (FAILED(hr))
    {
        skip("FreeThreadedXMLHTTP60 is not supported.\n");
        return;
    }
    ...
    hr = IXMLHTTPRequest2_Send(xhr, NULL, 0);
    ok(hr == S_OK, "Unexpected hr %#lx.\n", hr);
