...
static const BYTE timeValidV1CRLWithIssuerAndEntry[] = {
0x30,0x60,0x30,0x02,0x06,0x00,0x30,0x15,0x31,0x13,0x30,0x11,0x06,0x03,0x55,
0x04,0x03,0x13,0x0a,0x4a,0x75,0x61,0x6e,0x20,0x4c,0x61,0x6e,0x67,0x00,0x17,
0x0d,0x30,0x36,0x30,0x36,0x32,0x39,0x30,0x30,0x30,0x30,0x30,0x30,0x5a,0x17,
0x0d,0x30,0x37,0x30,0x36,0x32,0x39,0x30,0x30,0x30,0x30,0x30,0x30,0x5a,0x30,
0x25,0x30,0x23,0x02,0x10,0xeb,0x0d,0x57,0x2a,0x9c,0x09,0xba,0xa4,0x4a,0xb7,
0x25,0x49,0xd9,0x3e,0xb5,0x73,0x18,0x0f,0x31,0x36,0x30,0x31,0x30,0x31,0x30,
...
0xb0,0xca,0x16,0xc4,0xa4,0xc0,0xd6,0xe8,0xff,0x15,0x19,0xd1,0x30,0x61,0xf3,
0xef,0x9f };
static const BYTE chain29_0_crl[] = {
...
/* Thursday, Jan 1, 2004 */
static SYSTEMTIME jan2004 = { 2004, 1, 4, 1, 0, 0, 0, 0 };
/* Sunday, Oct 1, 2006 */
static SYSTEMTIME oct2006 = { 2006, 10, 0, 1, 0, 0, 0, 0 };
...
static void testGetCertChain(void)
{
    ...
    SystemTimeToFileTime(&jan2004, &fileTime);
    ret = pCertGetCertificateChain(NULL, cert, &fileTime, NULL, &para, 0, NULL,
     &chain);
    ...
    ret = pCertGetCertificateChain(NULL, cert, &fileTime, NULL, &para,
     CERT_CHAIN_REVOCATION_CHECK_CHAIN, NULL, &chain);
    ok(ret, "CertGetCertificateChain failed: %08x\n", GetLastError());
    if (ret)
    {
    ...
    CertAddEncodedCRLToStore(store, X509_ASN_ENCODING, verisignCRL,
     sizeof(verisignCRL), CERT_STORE_ADD_ALWAYS, NULL);
    ...
    cert = CertCreateCertificateContext(X509_ASN_ENCODING,
     selfSignedCert, sizeof(selfSignedCert));
    SystemTimeToFileTime(&oct2006, &fileTime);
    store = CertOpenStore(CERT_STORE_PROV_MEMORY, 0, 0,
     CERT_STORE_CREATE_NEW_FLAG, NULL);
    CertAddEncodedCertificateToStore(store, X509_ASN_ENCODING, selfSignedCert,
     sizeof(selfSignedCert), CERT_STORE_ADD_ALWAYS, NULL);
    CertAddEncodedCRLToStore(store, X509_ASN_ENCODING,
     timeValidV1CRLWithIssuerAndEntry, sizeof(timeValidV1CRLWithIssuerAndEntry),
     CERT_STORE_ADD_ALWAYS, NULL);
    ret = pCertGetCertificateChain(NULL, cert, &fileTime, store, &para,
     CERT_CHAIN_REVOCATION_CHECK_CHAIN, NULL, &chain);
    ok(ret, "CertGetCertificateChain failed: %08x\n", GetLastError());
     ...
     */
    cert = CertCreateCertificateContext(X509_ASN_ENCODING,
     chain29_1, sizeof(chain29_1));
    SystemTimeToFileTime(&oct2007, &fileTime);
    store = CertOpenStore(CERT_STORE_PROV_MEMORY, 0, 0,
     CERT_STORE_CREATE_NEW_FLAG, NULL);
    CertAddEncodedCertificateToStore(store, X509_ASN_ENCODING, chain29_0,
     sizeof(chain29_0), CERT_STORE_ADD_ALWAYS, NULL);
    CertAddEncodedCRLToStore(store, X509_ASN_ENCODING,
     chain29_0_crl, sizeof(chain29_0_crl), CERT_STORE_ADD_ALWAYS, NULL);
    ret = pCertGetCertificateChain(NULL, cert, &fileTime, store, &para,
     CERT_CHAIN_REVOCATION_CHECK_CHAIN, NULL, &chain);
    ok(ret, "CertGetCertificateChain failed: %08x\n", GetLastError());
    if (ret)
