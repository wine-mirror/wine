...
static void test_device_file_type(HANDLE device)
{
    ...
    file_type = GetFileType(device);
    todo_wine
    ok(file_type == FILE_TYPE_UNKNOWN, "Expected FILE_TYPE_UNKNOWN, got %lu\n", file_type);

    memset(&info, 0xcc, sizeof(info));
    memset(&io, 0xcc, sizeof(io));
    status = NtQueryVolumeInformationFile(device, &io, &info, sizeof(info), FileFsDeviceInformation);
    ok(status == STATUS_SUCCESS, "NtQueryVolumeInformationFile failed: %lx\n", status);
    ok(info.DeviceType == FILE_DEVICE_UNKNOWN, "Expected FILE_DEVICE_UNKNOWN (0x22), got %#lx\n", info.DeviceType);
    ok(info.Characteristics & FILE_AUTOGENERATED_DEVICE_NAME,
       "Expected FILE_AUTOGENERATED_DEVICE_NAME in Characteristics, got %#lx\n", info.Characteristics);
}

START_TEST(device)
{
    run_for_each_device(test_device_info);
    run_for_each_device(test_device_file_type);
    test_read_device();
