...
HRESULT WINAPI DllRegisterServer(void)
{
    ...
    if (FAILED(hr = __wine_register_resources()))
        return hr;
    if (FAILED(hr = MFTRegister(CLSID_CFrameRateConvertDmo, MFT_CATEGORY_VIDEO_EFFECT,
            (WCHAR *)L"Frame Rate Converter", MFT_ENUM_FLAG_SYNCMFT,
        ...
        return hr;
    if (FAILED(hr = DMORegister(L"Frame Rate Converter", &CLSID_CFrameRateConvertDmo, &DMOCATEGORY_VIDEO_EFFECT, 0,
        ...
        return hr;
    if (FAILED(hr = MFTRegister(CLSID_CColorControlDmo, MFT_CATEGORY_VIDEO_EFFECT,
            (WCHAR *)L"Color Control", MFT_ENUM_FLAG_SYNCMFT,
            ARRAY_SIZE(color_converter_mft_inputs), color_converter_mft_inputs,
            ARRAY_SIZE(color_converter_mft_outputs), color_converter_mft_outputs, NULL)))
        return hr;
    if (FAILED(hr = DMORegister(L"Color Control", &CLSID_CColorControlDmo, &DMOCATEGORY_VIDEO_EFFECT, 0,
            ARRAY_SIZE(color_converter_dmo_inputs), color_converter_dmo_inputs,
            ARRAY_SIZE(color_converter_dmo_outputs), color_converter_dmo_outputs)))
        return hr;
...
HRESULT WINAPI DllUnregisterServer(void)
{
    ...
    if (FAILED(hr = __wine_unregister_resources()))
        return hr;
    if (FAILED(hr = MFTUnregister(CLSID_CFrameRateConvertDmo)))
        return hr;
    if (FAILED(hr = DMOUnregister(&CLSID_CFrameRateConvertDmo, &DMOCATEGORY_VIDEO_EFFECT)))
        return hr;
    if (FAILED(hr = MFTUnregister(CLSID_CColorControlDmo)))
        return hr;
    if (FAILED(hr = DMOUnregister(&CLSID_CColorControlDmo, &DMOCATEGORY_VIDEO_EFFECT)))
        return hr;
